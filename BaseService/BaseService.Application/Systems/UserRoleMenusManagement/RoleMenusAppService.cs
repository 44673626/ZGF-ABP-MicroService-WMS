...
namespace BaseService.Systems.UserMenusManagement
{
    ...
    public class RoleMenusAppService : ApplicationService, IRoleMenusAppService
    {
        ...
        public async Task<ListResultDto<RoleMenusDto>> GetRoleMenus(string AppId)
        {
            var listMenu = new List<Menu>();
            var roleIds = await (await RoleRepository.GetDbSetAsync()).Where(_ => CurrentUser.Roles.Contains(_.Name)).Select(_ => _.Id).ToListAsync();
            var roleMenus = await (await _roleMenuRepository.GetQueryableAsync()).Where(_ => roleIds.Contains(_.RoleId)).Select(_ => _.MenuId).ToListAsync();
            var menus = await _menuRepository.GetListAsync(_ => _.CategoryId == 1 && string.IsNullOrEmpty(_.AppId) && roleMenus.Contains(_.Id));
            listMenu.AddRange(menus);//接入公共部分
            if (!string.IsNullOrEmpty(AppId))
            {
                var menus1 = await _menuRepository.GetListAsync(_ => _.CategoryId == 1 && _.AppId == AppId && roleMenus.Contains(_.Id));
                listMenu.AddRange(menus1);
            }
            var root = listMenu.Where(_ => _.Pid == null).OrderBy(_ => _.Sort).ToList();
            return new ListResultDto<RoleMenusDto>(LoadRoleMenusTree(root, listMenu));
        }

  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :xs="9" :sm="6" :md="5" :lg="4" :xl="4">
        ...
        <el-tree :data="orgData" :load="getOrgs" :props="defaultProps" :expand-on-click-node="true" highlight-current lazy
      ...
      <el-col :xs="15" :sm="18" :md="19" :lg="20" :xl="20">
        ...
        <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :title="formTitle" @close="cancel()"
          ...
          <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="70px">
            ...
            <el-form-item label="所属机构" style="display:inline" prop="organizationIds">
              <treeselect :multiple="false" v-model="form.organizationId" :load-options="loadOrgs" :options="orgs"
                style="width: 450px;line-height:18px" placeholder="请选择所属机构" noOptionsText="暂无数据" noChildrenText="暂无数据"
                noResultsText="暂无数据" />
            </el-form-item>
        ...
        <el-table ref="multipleTable" v-loading="listLoading" :data="list" size="small" style="width: 100%;"
          ...
          <el-table-column label="操作" align="center" width="125">
            <template slot-scope="{row}">
              <el-button type="primary" size="mini" @click="handleUpdate(row)" v-permission="['AbpIdentity.Users.Update']"
                icon="el-icon-edit" />
              <el-button type="danger" size="mini" @click="handleDelete(row)" :disabled="row.userName === 'admin'"
                v-permission="['AbpIdentity.Users.Delete']" icon="el-icon-delete" />
            </template>
          </el-table-column>
        </el-table>
  ...
  methods: {
    ...
    fetchData(id) {
      ...
      this.$axios.gets("/api/base/user/" + id).then(response => {
        ...
        this.form = response;
        if (response.organizationIds.length > 0) {
          this.form.organizationId = response.organizationIds.join(",")
        }
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.organizationId) {
            this.form.organizationIds.push(this.form.organizationId)
          }

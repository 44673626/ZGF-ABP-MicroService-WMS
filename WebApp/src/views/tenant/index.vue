    ...
    >
      ...
      >
        ...
        <el-form-item label="管理员邮箱" prop="adminEmailAddress" v-if="!isEdit">
          <el-input v-model="form.adminEmailAddress" style="width: 350px;" autocomplete="new-text" />
        </el-form-item>
        <el-form-item label="管理员密码" prop="adminPassword" v-if="!isEdit">
          <el-input  type="password" v-model="form.adminPassword" style="width: 350px;" autocomplete="new-password"/>
        </el-form-item>
      </el-form>
  ...
  data() {
    return {
      rules: {
        ...
        adminEmailAddress: [{ required: true, message: "请输入租户名", trigger: "blur" },
        { validator:validateEmail, message: "请输入正确的邮箱地址", trigger: "blur" }],
        adminPassword: [{required: true, message: "请输入租户名", trigger: "blur" },
            ...
            this.$axios
              ...
              .then((response) => {
                ...
                if (response.success && response.id) {
                  this.$axios.gets("/api/BaseData/ChangeTenant/ChangeConnection?guid=" + response.id)
                    .then(res2 => {
                      this.$axios.gets("/api/BaseData/CreateTenantDB/CreateTenDatabase?guid=" + response.id)
                          .then(res3 => {

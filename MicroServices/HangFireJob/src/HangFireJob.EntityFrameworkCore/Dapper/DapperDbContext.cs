...
namespace HangFireJob.Dapper
{
    ...
    public class DapperDbContext
    {
        ...
        public DapperDbContext(IConfiguration configuration)
        {
            ...
            _connectionString2 = configuration.GetConnectionString(HangFireJobDbProperties.ConnectionStringName);
        }
        ...
        /// <returns></returns>
        public async Task<IEnumerable<T>> QueryAsync<T>(string sql, object param = null, DatabaseType databaseType = DatabaseType.Default)
        {
        ...
        /// <param name="databaseType"></param>
        /// <returns></returns>
        public async Task<IEnumerable<T>> ExecuteProcedureAsync<T>(string procedureName, object param = null, DatabaseType databaseType = DatabaseType.Default)
        {
            var connectionString = databaseType == DatabaseType.Default ? _connectionString1 : _connectionString2;
            using (var connection = CreateConnection(connectionString))
            {
        ...
        /// <param name="databaseType"></param>
        /// <returns></returns>
        public async Task<int> ExecuteProcedureNonQueryAsync(string procedureName, object param = null, DatabaseType databaseType = DatabaseType.Default)
        {
            var connectionString = databaseType == DatabaseType.Default ? _connectionString1 : _connectionString2;
            using (var connection = CreateConnection(connectionString))
            {
